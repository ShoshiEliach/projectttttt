...
extern "C"
{
  ...
  void initializeList(const char *id)
  {
    std::string id_str(id);

    if (waiterListMap.find(id_str) != waiterListMap.end())
    {
      waiterListMap[id_str] = {};
    }
  }
  ...
  const char *WaitingLongTime()
  {
    auto currentTime = std::chrono::system_clock::now();
    const char *ans = "not";
    for (const std::string &id : ids)
    {
      if (waiterListMap.find(id) != waiterListMap.end() && !waiterListMap[id].empty())
      {
        const auto &waiter = waiterListMap[id].front(); // Get the first element in the list

        auto waiterTimePoint = std::chrono::time_point<std::chrono::system_clock>(std::chrono::seconds(waiter.timeValue));

        auto timeDiff = std::chrono::duration_cast<std::chrono::minutes>(currentTime - waiterTimePoint).count();
        if (timeDiff >= 2)
        {
          const char *id_char = id.c_str();

          return id_char; // Return the ID of the list immediately when condition is met
        }
      }
    }
    return ans;
  ...
  extern "C"
  {
    void countWaiters(std::list<Waiter> l)
    {
